...
function get_projects(){
    $projectsArray = array();
    $conn = DB_connectie();
    $sql = "SELECT Id, Titel, Aanmaakdatum, Straat, Omschrijving FROM tblprojecten WHERE Zichtbaar = 1";
    $result = $conn->query($sql);
    if(!DB_error($result, $conn)){
...
function get_popular_projects($amount){
    $projectsArray = array();
    $conn = DB_connectie();
    $sql = "SELECT project.Id, project.Titel, project.Aanmaakdatum, project.Straat, project.Omschrijving FROM tblprojecten project join tblprojectgebruikers user on project.Id = user.ProjectId WHERE project.Zichtbaar = 1 GROUP BY project.Titel ORDER BY count(user.ProjectId) desc LIMIT ?";
...
function get_project_category(){
    ...
    $type = 1;
    $conn = DB_connectie();
    $sql = "SELECT Id, Naam FROM tblcategorieen WHERE Soort = ?";
    $stmt = $conn->prepare($sql);
    ...
    if(!DB_error($result, $conn)){
        if ($result->num_rows > 0) {
            while($row = $result->fetch_array(MYSQL_ASSOC)){
                $project_categoryArray[] = project_category_item($row["Id"], $row["Naam"]);
            }
        }
    }
...
function get_event_category(){
    ...
    if(!DB_error($result, $conn)){
        if ($result->num_rows > 0) {
            while($row = $result->fetch_array(MYSQL_ASSOC)){
                $event_categoryArray[] = project_category_item($row["Id"], $row["Naam"]);
            }
        }
    }
...
function get_ad_category(){
    $ad_categoryArray = array();
    $type = 3;
    $conn = DB_connectie();
    $sql = "SELECT Id, Naam FROM tblcategorieen WHERE Soort = ?";
    $stmt = $conn->prepare($sql);
    ...
    if(!DB_error($result, $conn)){
        if ($result->num_rows > 0) {
            while($row = $result->fetch_array(MYSQL_ASSOC)){
                $ad_categoryArray[] = project_category_item($row["Id"], $row["Naam"]);
            }
        }
    }
    return $ad_categoryArray;
}

function get_article_category(){
    $article_categoryArray = array();
    $type = 4;
    $conn = DB_connectie();
    $sql = "SELECT Id, Naam FROM tblcategorieen WHERE Soort = ?";
    $stmt = $conn->prepare($sql);
    ...
    if(!DB_error($result, $conn)){
        if ($result->num_rows > 0) {
            while($row = $result->fetch_array(MYSQL_ASSOC)){
                $article_categoryArray[] = project_category_item($row["Id"], $row["Naam"]);
            }
        }
    }

    return $article_categoryArray;
}
...
function get_events(){
    $eventsArray = array();
    $conn = DB_connectie();
    $sql = "SELECT Id, Titel, Startdatum, Straat, Omschrijving FROM tblevents WHERE Zichtbaar = 1";
    $result = $conn->query($sql);
    if(!DB_error($result, $conn)){
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
...
function get_new_events($amount){
    $eventsArray = array();
    $conn = DB_connectie();
    $sql = "SELECT Id, Titel, Startdatum, Straat, Omschrijving FROM tblevents WHERE Zichtbaar = 1 ORDER BY Id DESC LIMIT ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $amount);
    $stmt->execute();
    $result = $stmt->get_result();
...
function GetGebruikerFull($gebruikersNaam)
{
	$gebruikerDetailArray = array();
    $gebruikerProjectenArray = array();
    $gebruikerEventsArray = array();
    $gebruikerArtikelsArray = array();
    $gebruikerZoekertjesArray = array();

    $conn = DB_connectie();
    $sql = "SELECT s.Gemeente as Stad, g.Gebruikersnaam, g.Geboortedatum, g.Biografie, tblFotos.Url as Foto FROM tblgebruikers g
    		left join tblFotos on tblFotos.Id = g.Foto
    		inner join tblSteden s on s.Id = g.PlaatsId WHERE g.Gebruikersnaam = ?";
    ...
    if(!DB_error($result, $conn)){
        if ($result->num_rows > 0) {
            $row = $result->fetch_array(MYSQLI_ASSOC);
            $gebruikerDetailArray = gebruiker_item_full($row["Stad"], $row["Gebruikersnaam"], $row["Geboortedatum"], $row["Biografie"], $row["Foto"]);
        }
    }
    return array($gebruikerDetailArray, $gebruikerProjectenArray, $gebruikerEventsArray, $gebruikerArtikelsArray, $gebruikerZoekertjesArray);
}
...
function get_articles($amount){
    ...
    $sql = "SELECT tblartikels.*, tblgebruikers.gebruikersnaam as User, tblcategorieen.naam as Categorie FROM tblartikels 
            ...
            WHERE tblArtikels.Zichtbaar = ?
            ORDER BY tblartikels.Aanmaakdatum DESC LIMIT ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii",$bool, $amount);
    $stmt->execute();
    $result = $stmt->get_result();
    if(!DB_error($result, $conn)){
        if ($result->num_rows > 0) {
...
function DB_error($result, $conn){
    if(!$result){
        die('Something is wrong with the Query: ' . $conn->error);
        return true;
    }
    return false;
}
